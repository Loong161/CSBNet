...
class CSFF(nn.Module):
    def __init__(self, n_feat, kernel_size, reduction, bias, act):
        ...
        self.content = nn.Sequential(nn.Conv2d(n_feat, n_feat // reduction, 1, padding=0, bias=bias),
                                 nn.ReLU(inplace=True),
                                 nn.Conv2d(n_feat // reduction, n_feat, 1, padding=0, bias=bias))
        self.spacial = nn.Sequential(ChannelPool(), conv(2, 1, kernel_size=1))
        self.conv = nn.Sequential(conv(n_feat, n_feat, kernel_size=1), act, conv(n_feat, n_feat, kernel_size))
        self.FAM = FAM(n_feat)

    def forward(self, x1, x2):
        x = x1 + x2
        x = self.conv(x)
        w1 = self.sigmoid(self.content(x))
        w2 = self.sigmoid(self.spacial(x))
        x1 = w1 * x1
        x2 = w2 * x2
        x = self.FAM(x1, x2)
        return x
...
class SIRNet(nn.Module):
    def __init__(self, n_feat, bias, kernel_size, reduction, act, n_rdb=4, growthRate=32):
...
class CSBNet(nn.Module):
    def __init__(self, in_c=3, out_c=3, n_feat=32, kernel_size=3, bias=False, reduction=4, act=nn.PReLU()):
        ...
        self.encoder = nn.Sequential(CAB(n_feat * 4, n_feat * 4, kernel_size, reduction, bias=bias, act=act),
                                     CAB(n_feat * 4, n_feat * 4, kernel_size, reduction, bias=bias, act=act))

        self.decoder = nn.Sequential(CAB(n_feat, n_feat, kernel_size, reduction, bias=bias, act=act),
                                     CAB(n_feat, n_feat, kernel_size, reduction, bias=bias, act=act))
        ...
        self.sirnet = SIRNet(n_feat, bias, kernel_size, reduction, act)
        self.conv = nn.ModuleList([
            conv(n_feat * 4, n_feat * 2, kernel_size=1),
            conv(n_feat * 8, n_feat * 4, kernel_size=1)
        ])
        self.CSFF = CSFF(n_feat, kernel_size, reduction, bias, act)
        self.tail = nn.Sequential(
            CAB(n_feat, n_feat, kernel_size, reduction, bias=bias, act=act),
            conv(n_feat, out_c, kernel_size, bias=bias))

    def forward(self, x):
        x_2 = F.interpolate(x, scale_factor=0.5)
        x_4 = F.interpolate(x_2, scale_factor=0.5)

        x_2_focus = self.DF1(x)  # 2
        x_4_focus = self.DF2(x_2)  # 4
        ...
        x_de_1 = self.decoder(x_up_1)

        encoder_outs = [x_en_1, x_en_2, x_en_4]  # 1 2 4
        decoder_outs = [x_de_1, x_de_2, x_de_4]  # 1 2 4

        feat_ors = self.sirnet(x1, encoder_outs, decoder_outs)
        # x_1_out = torch.cat((feat_ors, x_de_1), 1)
        x_1_out = self.CSFF(x_de_1, feat_ors)
