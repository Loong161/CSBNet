...
class CIRNet(nn.Module):
    def __init__(self, n_feat, kernel_size, reduction, bias, act):
        ...
        self.SAM = nn.ModuleList([
            SAM(n_feat * 4, kernel_size, bias),
            SAM(n_feat * 2, kernel_size, bias)
        ])
    ...
    def forward(self, param):
        ...
        z = self.FAM[0](x_down_2, x2)  # 2
        x_en_2, x_down_4 = self.Encoder[1](z)  # 2, 4

        z = self.FAM[1](x_down_4, x4)  # 4
        x_en_4 = self.feat_extract_1(z)  # 4

        z, x_4_out = self.SAM[0](x_en_4, x_4)  # 4
        x_de_4, x_up_2 = self.Decoder[0](z, x_en_2)  # 2

        z, x_2_out = self.SAM[1](x_up_2, x_2)  # 2
